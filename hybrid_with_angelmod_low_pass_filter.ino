...
volatile uint32_t attitudeCounter = 0;
...
static inline float rad_wrap(float x)
{
    const float full_rot = 2.0f * PI;
...
static ImuFilter filter_init(const float alpha)
{
    ImuFilter filter = {.alpha = alpha};
    return filter;
}


static Vec3 filter_update(ImuFilter* filter, const Vec3* acc, const Vec3* gyro, const float dt) {
  Vec3 rpy = filter->rpy;

  // Predict
  float g_r = rad_wrap(filter->rpy.x + ((gyro->x - filter->gyro_bias.x) * dt));
  float g_p = rad_wrap(filter->rpy.y + ((gyro->y - filter->gyro_bias.y) * dt));
  float g_y = rad_wrap(filter->rpy.z + ((gyro->z - filter->gyro_bias.z) * dt));

  // Apply measurement fix if no external acceleration
  // if (fabsf(vec3_mag(*acc) - 9.81f) < 1.0f) {
    if (true) {
    float a_r = atan2f(acc->y, acc->z);
    float a_p = atan2f(-acc->x, sqrt((acc->y * acc->y) + (acc->z * acc->z)));

    //  Serial.println("ACC UPDATE");

    // Complementary filter
    rpy.x = (filter->alpha * g_r) + ((1.0f - filter->alpha) * a_r);
    rpy.y = (filter->alpha * g_p) + ((1.0f - filter->alpha) * a_p);
    rpy.z = g_y;

    // Update bias
    const float biasGain = 0.002f;

    filter->gyro_bias.x += biasGain *
        (gyro->x - ((rpy.x - filter->rpy.x) / dt) - filter->gyro_bias.x);

    filter->gyro_bias.y += biasGain *
        (gyro->y - ((rpy.y - filter->rpy.y) / dt) - filter->gyro_bias.y);
...
void setup() {
  ...
  // ── 2. IMU setup ───────────────────────────────────────
        ...
        filter = filter_init(0.75f);

        filter.rpy.x = currentRoll * DEG_TO_RAD;
        filter.rpy.y = currentPitch * DEG_TO_RAD;
        filter.rpy.z = 0.0f;
...
if (micros() - lastPrintUs >= 1000000UL)
{
    lastPrintUs += 1000000UL;

    Serial.print("Loops/sec = ");
    Serial.print(loopCounter);

    Serial.print("   Attitude/sec = ");
    Serial.println(attitudeCounter);
...
//     }
    ...
    gyro.x = gyroX * DEG_TO_RAD;
    gyro.y = gyroY * DEG_TO_RAD;
    gyro.z = gyroZ * DEG_TO_RAD;

    Vec3 rpy = filter_update(&filter, &acc, &gyro, dt);

    // Debug prints
// Serial.print("Roll: ");
// Serial.print(rpy.x * RAD_TO_DEG);

// Serial.print("\tBias: ");
// Serial.print(filter.gyro_bias.x * RAD_TO_DEG);

// Serial.print("\t|a|: ");
// Serial.println(vec3_mag(acc));

    currentRoll  = rpy.x * RAD_TO_DEG;
    currentPitch = rpy.y * RAD_TO_DEG;
    currentYaw   = rpy.z * RAD_TO_DEG;
}
...
void computePID() {
    ...
    // ---------- Serial Plotter ----------
      ...
      // Serial.print(gyroX);
      // Serial.print('\t');
      // Serial.println(filter.gyro_bias.x * RAD_TO_DEG);
}
